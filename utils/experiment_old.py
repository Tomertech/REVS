                ...
                for steering_coeff in steering_coeffs:
                    for seed in seeds:
                        ...
                        if config.save_model:
                            path = f"{PROJECT_PATH}/delpii/models/unlearn/RMU/{model_type.lower()}/{exp_type.lower()}"
                            save_model(updated_model, tokenizer, path, exp_config, df_dict)
            ...
            for beta in betas:
                for seed in seeds:
                    ...
                    if config.log_wandb:
                        ...
                        wandb.log({f"{model_type.upper()} Aggregated Results": res_plot})
                    if config.save_model:
                        path = f"{PROJECT_PATH}/delpii/models/unlearn/NPO/{model_type.lower()}/{exp_type.lower()}"
                        save_model(updated_model, tokenizer, path, exp_config, df_dict)
