...
def layer_stats(
    ...
    def get_ds():
        ...
        # Check if 'n_positions' attribute exists in model.config
        if hasattr(model.config, 'n_positions'):
            maxlen = model.config.n_positions // 2
        # If 'n_positions' doesn't exist, check if 'max_position_embeddings' exists
        elif hasattr(model.config, 'max_position_embeddings'):
            maxlen = model.config.max_position_embeddings // 2
        else:
            raise AttributeError("Both 'n_positions' and 'max_position_embeddings' are missing from model.config")
